...
ggplot() +
  ...
  geom_point(data = traditional, aes(x = x + 1, y = y), size = 22, color = "steelblue", alpha = 0.7) +
  geom_text(data = traditional, aes(x = x + 1, y = y, label = labels), size = 3.5) +

  # Unified approach - connected to central framework
  geom_point(data = unified, aes(x = x + 1, y = y), size = 22, color = "steelblue", alpha = 0.7) +
  geom_text(data = unified, aes(x = x + 1, y = y, label = labels), size = 3.5) +

  # Connections to central framework
  geom_segment(
    aes(
      x = unified$x + 1, y = unified$y + 0.3,
      xend = center$x + 1, yend = center$y - 0.3
    ),
    arrow = arrow(length = unit(0.2, "cm")), size = 0.7
  ) +

  # Central framework
  geom_point(data = center, aes(x = x + 1, y = y), size = 28, color = "darkred", alpha = 0.7) +
  geom_text(data = center, aes(x = x + 1, y = y, label = labels), size = 4, color = "white") +

  # Labels for the approaches
  annotate("text",
    x = 0, y = 1, label = "Traditional Approach:",
    hjust = 0, size = 5, fontface = "bold"
  ) +
  annotate("text",
    x = 0, y = 2, label = "Unified Approach:",
    hjust = 0, size = 5, fontface = "bold"
  ) +
...
## Example 2: Independent t-test as a Linear Model {.smaller}
...
## Dummy Coding: How Categorical Variables Work in Linear Models {.smaller}
...
ANOVA (Analysis of Variance) is an extremely common statistical test.

Conceptually, it is similar to a t-test, but instead of comparing a single group against a baseline (one-sample t-test) or comparing two groups (independent t-test), it allows us to compare multiple groups.

Multiple variations of ANOVA exist as well, such as MANOVA (multivariate ANOVA), CANOVA (categorical ANOVA), and so on. ANOVA is often convenient as it presents the results in a consistent and simpler to interpret format.

But, to understand what it means statistically, again we can consider it as a specific application of the General Linear Model.

## Example 3: ANOVA as a Linear Model {.smaller}
...
## Example 4: Multiple Regression as a Linear Model {.smaller}
...
The 3D visualization shows how multiple regression works with two continuous predictors: 
- Each blue dot represents a student's data (study hours, previous grades, and test score) 
- The model creates a "plane" in this 3D space that best fits the data points 
- The plane's position at y-axis=0 represents β₀ (the intercept) 
- The plane's slope in the x₁ direction represents β₁ (effect of study hours) 
- The plane's slope in the x₂ direction represents β₂ (effect of previous grades)
...
## Question 1: Is the average tenure different from the standard? {.smaller}
...
**Linear Model**: $\text{tenure} = \beta_0 + \varepsilon$
...
## Question 2: Is there a gender difference in intention to quit? {.smaller}

::: columns
::: {.column width=50%}

**Question**: Is there a gender difference in the intention to quit?

**Linear Model**: $\text{intentionto_quit} = \beta_0 + \beta_1 \text{gender} + \varepsilon$
:::
...
::: notes
Now let's address our second question: Is there a gender difference in the intention to quit?
...
intention to quit = β₀ + β₁×gender + ε

where gender is coded as 0 for females and 1 for males.

First, we run a traditional independent t-test using the t.test() function. The results show that males have a slightly lower average intention to quit (2.90) compared to females (2.97), and this difference is not statistically significant (p = 0.4).

Next, we run the same test as a linear model using lm(). Here: 

- The intercept (β₀) is 2.97, which is the average intention rating for females (the reference group) 
- The coefficient for genderMale (β₁) is -0.07, which is the difference between male and female intention ratings 
- The t-value and p-value for this coefficient are identical to those from the independent t-test
...
The advantage of the linear model approach is that it gives us not just the test of difference but also the estimate of how large that difference is (-0.07 intention rating points), which is directly interpretable.
...
## Question 3: Does the intention to quit differ across job roles? {.smaller}

::: columns

::: {.column width=50%}

**Question**: Does the intention to quit differ across job roles?

**Linear Model**: $\text{intentionto_quit} = \beta_0 + \beta_1 \text{role}_1 + \beta_2 \text{role}_2 + ... + \varepsilon$
:::
...
## Question 4: What factors predict the intention to quit? {.smaller}

**Question**: What factors predict the intention to quit?

**Linear Model**: $\text{intentionto_quit} = \beta_0 + \beta_1 \text{gender} + \beta_2 \text{experience} + \beta_3 \text{performance} + \varepsilon$
...
How does *controlling* for the salary grade impact the model?
...
## Combining Different Types of Predictors {.smaller}
