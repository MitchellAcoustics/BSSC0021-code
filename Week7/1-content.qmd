...
# Create a visualization of the impact of statistical tests
test_data <- tibble(
  Test = c("t-test", "t-test", "ANOVA", "ANOVA", "Regression", "Regression"),
  Type = c("One-sample", "Independent", "One-way", "Two-way", "Simple", "Multiple"),
  Application = c("QC Testing", "A/B Testing", "Fertilizer", "Retail", "Staffing", "Housing"),
  Impact = c(9, 8, 7, 8.5, 7.5, 9)
)

ggplot(test_data, aes(x = reorder(paste(Type, "-", Test), Impact), y = Impact, fill = Test)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  labs(
    title = "Business Impact of Statistical Tests",
    y = "Relative Impact (1-10 scale)",
    x = ""
  ) +
...
1. One-sample t-test in Pharmaceutical QC testing 500mg active ingredient tablets → FDA compliance or recalls
2. Independent t-test in A/B testing comparing two website designs → Millions in potential revenue
3. One-way ANOVA in comparing yields across four fertilizer types → Optimal crop production
4. Two-way ANOVA in testing how store location and pricing affect sales → Retail strategy optimization
5. Simple regression in predicting staffing needs based on foot traffic → Labor cost management
6. Multiple regression in house price estimation using multiple factors → Accurate property valuation
...
ggplot() +
  ...
  geom_point(data = subset(plot_data, approach == "Traditional"), 
             aes(x = x, y = y), size = 8, color = "darkblue") +
  # Unified approach: connected points
  geom_point(data = subset(plot_data, approach == "Unified"), 
  ...
  geom_text(data = plot_data, 
            ...
            color = "white", size = 3) +
  # Add approach labels
  annotate("text", x = 2.5, y = 1.5, label = "Traditional: Isolated Tests", 
           size = 4, color = "darkblue") +
  annotate("text", x = 2.5, y = 3.5, label = "Unified: Connected Framework", 
...
nodes <- tibble(
  ...
  label = c("General Linear Model", "Regression", "ANOVA", "t-tests", 
...
ggplot(df, aes(x, y)) +
  ...
  annotate("segment", x = 2, xend = 2, y = 2, yend = 4, 
           arrow = arrow(length = unit(0.3, "cm")), color = "darkred") +
  annotate("text", x = 8, y = 7, label = expression(beta[1]), color = "darkred", size = 5) +
  annotate("segment", x = 8, xend = 9, y = 6.5, yend = 6.5,
           arrow = arrow(length = unit(0.3, "cm")), color = "darkred") +
  annotate("segment", x = 9, xend = 9, y = 6.5, yend = 7.5,
           arrow = arrow(length = unit(0.3, "cm")), color = "darkred") +
  theme_minimal() +
  labs(title = "Visual Representation",
       x = "Predictor (x)",
       y = "Outcome (y)")
```
::::
...
ggplot(sample_data, aes(x = x, y = y)) +
  ...
  geom_ribbon(aes(ymin = mean(y) - 2*sd(y)/sqrt(length(y)), 
                  ymax = mean(y) + 2*sd(y)/sqrt(length(y))),
              alpha = 0.2, fill = "darkred") +
  # Add annotation for intercept
  annotate("text", x = 1.3, y = mean(y) + 0.5, 
...
ggplot(data, aes(x = group, y = y, color = group)) +
  ...
  annotate("text", x = 1, y = mean(data$y[data$group == "A"]) - 0.5, 
  ...
  annotate("text", x = 1.5, y = mean(data$y[data$group == "A"]) + 1, 
...
plot_ly() |>
  add_markers(data = multi_data, x = ~x1, y = ~x2, z = ~y, 
              marker = list(color = "blue", size = 6, opacity = 0.8),
              name = "Data points") |>
  add_surface(x = x1_range, y = x2_range, z = z_matrix, 
...
model_summary |>
  ...
  labs(title = "Effect of Predictors on Salary",
...
ggplot(hr_data, aes(x = tenure, y = salarygrade, color = gender)) +
  ...
  annotate("text", x = 16, 
           y = mean(c(predict(hr_model, newdata = data.frame(tenure = 15, evaluation = mean(hr_data$evaluation), gender = "Female")),
